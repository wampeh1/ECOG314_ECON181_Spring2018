...
Update the y axis so that is goes from 0-175,000 in 25,0000 increments

```{r}
acs_2016_cleaned %>%
  mutate(rounded_age = plyr::round_any()) %>%
  group_by() %>%
...
acs_2016_cleaned %>%
  group_by() %>%
  summarise(mean_income = wtd.mean(, , na.rm = F)) %>%
